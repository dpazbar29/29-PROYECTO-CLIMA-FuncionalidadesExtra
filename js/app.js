...
let ultimaBusqueda = { ciudad: null, pais: null, lat: null, lon: null };
...
function actualizarHistorial(ciudad, pais) {
    const historial = document.querySelector('#historial');
    const unidad = mostrarEnFahrenheit ? 'Fahrenheit' : 'Celsius';
    const ultimaTemp = mostrarEnFahrenheit ? kelvinToFahrenheit(ultimaBusqueda.temp) : kelvinToCentigrados(ultimaBusqueda.temp);

    const nuevoItem = `${ciudad}, ${pais} (${unidad}, ${ultimaTemp}°)`;
...
function consultarAPI(ciudad, pais) {
    ultimaBusqueda = { ciudad, pais, lat: null, lon: null };
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            ultimaBusqueda.temp = data.main.temp;
            mostrarClima(data);
            actualizarHistorial(ciudad, pais);
            
        })
        .catch(() => {
            monstarError("Error en la consulta. Intenta de nuevo más tarde.");
        });
}
...
function mostrarClima(clima) {
    ...
    const simboloGrados = mostrarEnFahrenheit ? '&#8457;' : '&#8451;';
    ...
    ultimaBusqueda.temp = temp;

    const actual = document.createElement("p");
    actual.innerHTML = `${clima.name}, ${country} ${temperatura} ${simboloGrados}`;
    actual.classList.add("text-6xl");

    const tempMin = document.createElement("p");
    tempMin.innerHTML = `Mínima: ${temperaturaMin} ${simboloGrados}`;
    tempMin.classList.add("text-xl");

    const tempMax = document.createElement("p");
    tempMax.innerHTML = `Máxima: ${temperaturaMax} ${simboloGrados}`;
...
botonCambioUnidad.addEventListener('click', () => {
    ...
    if (ultimaBusqueda.ciudad && ultimaBusqueda.pais) {
        consultarAPI(ultimaBusqueda.ciudad, ultimaBusqueda.pais);
    } else if (ultimaBusqueda.lat !== null && ultimaBusqueda.lon !== null) {
        consultarAPIConCoordenadas(ultimaBusqueda.lat, ultimaBusqueda.lon);
    } else {
        monstarError("No hay una búsqueda previa disponible");
    }
});
...
function consultarAPIConCoordenadas(lat, lon) {
    ultimaBusqueda = { ciudad: null, pais: null, lat, lon };
    const API_Key = "3772a6f00739f6fea4645469c4abda23";
